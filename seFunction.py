...
class mainTable:
    ...
    def getAttname(self,index):
        print(self.attrTable[index].name)
...
def insertMainTable(name, Type, typeMod, extends, implements):
    for j in  implements:
        for i in mainTable_:
            fname = i.getName()
            maintype = i.getMainType()
            if fname == j :
                if maintype != "implements":
                    sms = fname + " is not interface ---"
                    return False, sms


    sms =  redeclarationError(extends)
    if lookupMainTable(name) == False:
        if extends != "" and lookupMainTable(extends) == False:
            smms = "------------cannot extends:" + extends+ " not declare---------" 
            return False, smms
        elif extends != "":
            for i in mainTable_:
                fname = i.getName()
                ftype = i.getTypeMod()
                maintype = i.getMainType()
                # print("main type", maintype,fname)
                if (fname == extends) and (ftype == "final"):
                    smms = "------------cannot extend:" + extends+ " bcz final---------"
                    extends = ""
                    return False, smms
                if (fname == extends) and (maintype != Type):
                    smms = "------------cannot extend:" + extends+ " bcz type mismatch---------"
                    extends = ""
                    return False, smms
...
def lookupAttributeForTypeDetail(name, ofName):
    x = lookupMainTable(ofName)
    y = next((j for j in x.attrTable if j.name == name), "")
    if y == "":
        return False
    elif "->" not in y.type:
        return y.type , y.accessMod, y.static, y.abstract, y.final
    # elif "->" in y.type:
    #     return y.type[-1] , y.accessMod, y.static, y.abstract, y.final
    else:
        return False ,"null","null","null","null"
def lookupAttributeForFuncTypeDetail(name, ofName):
    x = lookupMainTable(ofName)
    y = next((j for j in x.attrTable if j.name == name), "")
    if y == "":
        return False
        ...
        return False ,"null","null","null","null"
# def superCheck(value , ofName):
#     x = lookupMainTable(ofName)
#     if x.extends !="" :
#         return x.extends
#     if x ==False or x.type == "":
